...
#' @param coldata Original sample metadata
#' @param config Configuration list
save_dds_with_metadata <- function(dds, experiment_name, coldata = NULL, config = NULL) {
  # Create DDS directory
  dds_dir <- file.path("outputs", "dds")
  dir.create(dds_dir, recursive = TRUE, showWarnings = FALSE)
  
  # Save DDS object
  dds_path <- file.path(dds_dir, paste0(experiment_name, "_dds.RDS"))
  ...
  metadata <- list(
    ...
    design_formula = as.character(design(dds)),
    sample_names = colnames(dds),
    # DESeq2 run parameters
    deseq_version = packageVersion("DESeq2"),
    size_factors = sizeFactors(dds),
    dispersions_summary = list(
      median = median(dispersions(dds), na.rm = TRUE),
      mad = mad(dispersions(dds), na.rm = TRUE)
    )
  )
  
  # Add original coldata if provided
  if (!is.null(coldata)) {
    metadata$original_coldata_summary <- list(
      n_samples_before_filtering = nrow(coldata),
      columns = colnames(coldata)
    )
  }
  
  # Add config info if provided  
  if (!is.null(config)) {
    metadata$analysis_config <- config$analysis
  }
  
  # Save metadata
  metadata_path <- file.path(dds_dir, paste0(experiment_name, "_metadata.yaml"))
  write_yaml(metadata, metadata_path)
  
  cat(glue("💾 DDS saved: {dds_path}\n"))
  cat(glue("📋 Metadata saved: {metadata_path}\n"))
  
  return(list(dds_path = dds_path, metadata_path = metadata_path))
}
