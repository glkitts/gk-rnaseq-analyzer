...
# Final minimal report generation - Fixed symbols and correct output directory
# Reports saved to outputs/reports, temporary data to outputs/R
...
generate_minimal_summary <- function(experiment_name, res.l.all, dds, coldata) {
  ...
  # Build HTML content with fixed encoding
  html_content <- paste0(
    '<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  # Save HTML file to outputs/reports directory (FIXED)
  ...
  writeLines(html_content, output_file, useBytes = TRUE)  # Added useBytes for encoding
  
  cli_inform("Generated minimal summary: {basename(output_file)}")
  return(output_file)
}

#' Generate minimal comparison report - FIXED symbols and directory
generate_minimal_comparison <- function(comparison_name, comparison_data, experiment_name) {
  ...
  # Debug info
  cli_inform("Processing {comparison_name}: {nrow(results_de)} DE genes")
  ...
  # Build HTML content with FIXED symbols using HTML entities
  html_content <- paste0(
    '<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <ul>
    ...
    <li><strong>Significant (padj &le; 0.05):</strong> ', nrow(results_sig), '</li>
  ...
  <p><a href="', experiment_name, '_minimal_summary.html">&larr; Back to Summary</a></p>
  <p><em>Generated by Yildiz Lab RNA-seq Pipeline</em></p>
</body>
</html>')
...
#' Generate all minimal reports with optional data saving
generate_minimal_reports <- function(experiment_name, res.l.all, dds, coldata, contrast_list, save_data = TRUE) {
  
  cli_inform("Generating minimal reports for {experiment_name}...")
  
  # Optional: Save res.l.all to outputs/R for future use
  if (save_data) {
    r_dir <- here("experiments", experiment_name, "outputs", "R")
    if (!dir.exists(r_dir)) {
      dir.create(r_dir, recursive = TRUE)
    }
    
    res_file <- file.path(r_dir, paste0(experiment_name, "_res.l.all.RDS"))
    saveRDS(res.l.all, res_file)
    cli_inform("Saved results data: {basename(res_file)}")
  }
  ...
  cli_inform("Reports saved to: experiments/{experiment_name}/outputs/reports/")
