...
# Load plotly conditionally for HTML output
if (params$output_format == "html") {
  suppressPackageStartupMessages(library(plotly))
}

# Load data from temporary files
if (!is.null(params$temp_comp_file) && file.exists(params$temp_comp_file)) {
  comparison_data <- readRDS(params$temp_comp_file)
} else {
  stop("Comparison data file not found")
}

if (!is.null(params$temp_exp_file) && file.exists(params$temp_exp_file)) {
  experiment_info <- readRDS(params$temp_exp_file)
} else {
  stop("Experiment info file not found")
}

# Extract data for convenience
contrast <- comparison_data$contrast
results <- comparison_data$results
volcano_data <- comparison_data$volcano_data
gsea_data <- comparison_data$gsea_data
...
# Create interactive version for HTML if plotly is available
if (requireNamespace("plotly", quietly = TRUE)) {
  ...
  cat("**Interactive plot requires plotly package**\n")
}
```
