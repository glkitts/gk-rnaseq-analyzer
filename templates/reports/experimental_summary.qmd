...
# Get comparison names from the data
...
sample_info <- coldata %>%
  ...
  select(sample_id, everything(), -files)
...
lib_size_df <- tibble(
  ...
  left_join(as_tibble(coldata, rownames = "sample_id"), by = c("sample" = "sample_id"))
...
# Create blue color scheme for distance heatmap
blue_colors <- colorRamp2(
  seq(min(sample_dist_matrix), max(sample_dist_matrix), length = 9),
...
if (ncol(annotation_df) > 0) {
  ...
  ht <- Heatmap(
    ...
    clustering_distance_columns = sample_dists
  )
} else {
...
# Create summary table for significant genes
de_summary_sig <- map_dfr(names(res.l.all), function(comp_name) {
  res_all <- res.l.all[[comp_name]]$all
...
# Create summary table for DE genes
de_summary_DE <- map_dfr(names(res.l.all), function(comp_name) {
  res_all <- res.l.all[[comp_name]]$all
  ...
  tibble(
    comparison = comp_name,
    genes_tested = nrow(res_all),
    de_genes = nrow(res_DE),
    upregulated = sum(res_DE$log2FoldChange > 0, na.rm = TRUE),
    downregulated = sum(res_DE$log2FoldChange < 0, na.rm = TRUE),
    percent_DE = round(100 * nrow(res_DE) / nrow(res_all), 1)
  )
})

# Display significant genes table
de_summary_sig %>%
  rename(
  ...
  knitr::kable(
    caption = "Significant Genes Summary (padj threshold only)"
  )
```

### Differentially Expressed Genes

```{r de-summary-DE, echo=FALSE}
...
de_summary_DE %>%
  ...
  knitr::kable(
    caption = "Differentially Expressed Genes Summary (padj & log2FC thresholds)"
  )
```
...
[**🔗 Interactive Gene Expression Heatmap**](`r params$experiment_name`_interactive_heatmap.html)

*Note: This interactive heatmap may take a while to load if the total gene number present is large.*
:::

## Individual Comparison Links

```{r comparison-nav, results='asis', echo=FALSE}
comparison_links <- map_chr(comparison_names, function(comp) {
  glue::glue("- [{comp}]({comp}_report.html)")
})
