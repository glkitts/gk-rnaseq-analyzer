...
de_stats <- list(
  "DE Genes" = nrow(res_DE),
  "DE Upregulated" = sum(res_DE$log2FoldChange > 0, na.rm = TRUE),
  "DE Downregulated" = sum(res_DE$log2FoldChange < 0, na.rm = TRUE),
  "% DE" = round(100 * nrow(res_DE) / nrow(res_all), 1)
)
...
**Significant Genes (padj ≤ `r sig_threshold`):**
...
**Differentially Expressed Genes (padj ≤ `r sig_threshold` & |log₂FC| ≥ `r fc_threshold`):**
...
::: g-col-3
**`r de_stats$"DE Genes"`** DE Genes
:::

::: g-col-3
**`r de_stats$"DE Upregulated"`** Upregulated
:::

::: g-col-3
**`r de_stats$"DE Downregulated"`** Downregulated
:::
:::::::
...
::: g-col-4
**Percent significant:** `r summary_stats$"% Significant"`% | **Percent DE:** `r de_stats$"% DE"`%
:::
::::::
...
```{r gsea-all, results='asis'}
# Genesets to visualize in reports (edit this vector to add/remove)
report_genesets <- c("goBP", "yildiz.combined")

# Filter to available genesets that we want to report
genesets_to_plot <- intersect(report_genesets, names(gsea_results))

if (length(genesets_to_plot) == 0) {
  cat("No GSEA results available for this comparison.\n")
} else {
  for (gs_name in genesets_to_plot) {
    # Print subsection header
    cat("\n###", gs_name, "\n\n")

    gsea_data <- NULL
    gs_entry <- gsea_results[[gs_name]]

    if (!is.null(gs_entry) &&
        !is.null(gs_entry$gsea_results) &&
        params$comparison_name %in% names(gs_entry$gsea_results)) {
      gsea_data <- gs_entry$gsea_results[[params$comparison_name]]
    }
    ...
    if (!is.null(gsea_data) && is.data.frame(gsea_data) && nrow(gsea_data) > 0) {
      ...
      if (nrow(sig_pathways) > 0) {
        ...
        color_min <- max(0.5, floor(padj_range[1] * 10) / 10)
        color_max <- ceiling(padj_range[2] * 10) / 10

        size_range <- range(lengths(sig_pathways$leadingEdge))
        size_min <- floor(size_range[1] / 10) * 10
        size_max <- ceiling(size_range[2] / 10) * 10
        ...
        p <- ggplot(sig_pathways, aes(
          ...
          labs(
            title = paste(gs_name, "Enrichment:", comparison_description),
            y = NULL,
        ...
        cat("No significant pathways found (padj \u2264 0.05 with >2 leading edge genes).\n\n")
      }
